...
export default function DeleteDoctorsModal({
  open,
  onCancel,
  doctorData,
  onSuccess,
  ...
  const { isLoading } = useSelector((state) => state.deletDoctors);

  const handleDelete = () => {
if (!doctorData?.id) return;
dispatch(deletDoctors(doctorData.id))      .unwrap()
      .then(() => {
        // 🌟 إظهار رسالة النجاح الخضراء اللطيفة في أعلى الشاشة
        message.success({
          content: "تم حذف الطبيب بنجاح!",
      ...
      .catch((err) => {
        ...
        message.error("عذراً، حدث خطأ أثناء محاولة حذف الدكتور.");
      });
  };
    ...
    >
      <div style={{ textAlign: "center", direction: "rtl" }}>
        ...
        <p style={{ color: "#666", fontSize: "14px", marginBottom: "24px" }}>
          هل أنت متأكد أنك تريد حذف الطبيب <br />
          <Text strong style={{ color: "#222", fontSize: "15px" }}>
"{doctorData?.name}"          </Text>؟ <br />
