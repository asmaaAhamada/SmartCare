...
import { deletDoctors } from "../../backend/slice/doctors/delet";
import { DeleteClinic } from "../../backend/slice/clinic/delet";
...
export default function DeleteClinicModal({
  ...
  const { isLoading } = useSelector((state) => state.DeleteClinic);

  const handleDelete = () => {
if (!clinic?.id) return;
dispatch(DeleteClinic(clinic.id))      .unwrap()
      .then(() => {
        // 🌟 إظهار رسالة النجاح الخضراء اللطيفة في أعلى الشاشة
        message.success({
          content: "تم حذف العيادة بنجاح!",
      ...
      .catch((err) => {
        console.error("فشلت عملية حذف العيادة:", err);
        // رسالة تنبيه في حال حدوث خطأ غير متوقع
        message.error("عذراً، حدث خطأ أثناء محاولة حذف العيادة.");
      });
  };
    ...
    >
      <div style={{ textAlign: "center", direction: "rtl" }}>
        ...
        <p style={{ color: "#666", fontSize: "14px", marginBottom: "24px" }}>
          هل أنت متأكد أنك تريد حذف العيادة <br />
